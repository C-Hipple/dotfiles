...
If you are unsure, try setting them in `dotspacemacs/user-config' first."
  ...
  (defun set-frame-stream-size ()
    (interactive)
    ;; For my 4k wide monitor
    (set-frame-size (selected-frame) 2460 1370 't))
  )
...
before packages are loaded."
  ...
  ;;   :ensure t)
  ;; (use-package shell-maker
  ;;   :vc (:fetcher github :repo xenodium/shell-maker))
    ...
    (let ((mode (symbol-name major-mode)))
      (cond
       ...
       ((or (string= mode "tsx-ts-mode")
            ...
            (string= mode "scss-mode"))
        (message "Formatting frontend with prettier")
        (prettier-prettify))
       (t
        (lsp-format-buffer)))))
...
\\<minibuffer-local-map>\\[next-history-element])."
  ...
  (defun open-next-actions-org ()
    ...
    (if (eq system-type 'gnu/linux)
        (find-file "~/personal_next_actions.org")
...
Operate on selected region on whole buffer."
  ...
  (with-eval-after-load 'code-review
    ...
    (define-key evil-motion-state-map (kbd ", g f") 'lsp-find-references)

    ;; you shouldn't need these but just in case
    (define-key evil-motion-state-map (kbd ", g R") 'diff-lsp-refresh)
    (define-key evil-motion-state-map (kbd ", g f") 'diff-lsp-fetch))
  ...
  (define-key evil-motion-state-map (kbd ", a m") 'gptel-menu)
  (define-key evil-motion-state-map (kbd ", a a") 'gptel-add)
  (define-key evil-motion-state-map (kbd ", a c") 'gptel) ;; chat
  (add-hook 'gptel-post-stream-hook 'gptel-auto-scroll)

  (with-eval-after-load 'gptel
    (add-to-list 'gptel-directives
                 '(review . "You are an expert code reviewer.  This code is a Pull request for a full stack web application.  Please review the pull request.  You can ignore CI failures, status, or comments from gemini, or comments that are marked as 'Comment by YOU'.  Make sure to note any code smells or non-idiomatic code you find.  Also check for bugs, or non-performant code.")))
